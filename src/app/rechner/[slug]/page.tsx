...
export default function RechnerPage({ params }: { params: { slug: string } }) {
  ...
  if (loading) return <div className="py-20 text-center text-stone-400">Lade Rechner...</div>;
  if (!rechner) return <div className="py-20 text-center">Rechner nicht gefunden.</div>;

  const materialliste = berechnet
    ? materialien.map((m) => ({ ...m, menge: getMenge(m.formel) })).filter((m) => m.menge > 0)
    : [];

  const gesamtpreis = materialliste.reduce((sum, m) => sum + m.menge * m.preis_ca, 0);
    ...
    <div className="max-w-3xl">
      ...
      <div className="bg-white border border-stone-200 rounded-xl p-6 mb-8">
        ...
        <button
          ...
          className="mt-6 w-full bg-amber-600 text-white font-semibold py-3 px-6 rounded-lg hover:bg-amber-700 transition-colors"
        >
          Berechnen
        </button>
      </div>
      ...
      {berechnet && (
        <>
              ...
              <tbody>
                {material.map((m) => (
                  <tr key={m.id} className="border-b border-stone-100">
                    ...
                    <td className="text-right py-3">{Math.round(m.menge)} {m.einheit}</td>
                    <td className="text-right py-3 text-stone-500">{formatPreis(m.menge * m.preis_ca)}</td>
          ...
          {werkzeuge.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-6">
              ...
              <div className="grid gap-3">
                {werkzeuge.map((w) => (
                  <div key={w.id} className="flex items-center justify-between bg-white rounded-lg p-3 border border-amber-100">
                    <div>
          ...
          <CommunityKosten seitenSlug={rechner.slug} />
        </>
      )}
    </div>
  );
